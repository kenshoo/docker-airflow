...
then
  ...
  sed -i "s#celery_result_backend = mysql+mysqldb://root:root@mysql/airflow#celery_result_backend = mysql+mysqldb://$MYSQL_USER:$MYSQL_PASSWORD@$MYSQL_HOST:$MYSQL_PORT/$MYSQL_DB#" "$AIRFLOW_HOME"/airflow.cfg
  sed -i "s#sql_alchemy_conn = mysql+mysqldb://root:rootw@mysql/airflow#sql_alchemy_conn = mysql+mysqldb://$MYSQL_USER:$MYSQL_PASSWORD@$MYSQL_HOST:$MYSQL_PORT/$MYSQL_DB#" "$AIRFLOW_HOME"/airflow.cfg
...
then
  ...
  sed -i "s#sql_alchemy_conn = mysql+mysqldb://root:root@mysql/airflow#sql_alchemy_conn = mysql+mysqldb://$MYSQL_USER:$MYSQL_PASSWORD@$MYSQL_HOST:$MYSQL_PORT/$MYSQL_DB#" "$AIRFLOW_HOME"/airflow.cfg
  sed -i "s#broker_url = redis://redis:6379/1#broker_url = redis://$REDIS_PREFIX$REDIS_HOST:$REDIS_PORT/1#" "$AIRFLOW_HOME"/airflow.cfg
  echo "Initialize database..."
  $CMD initdb
  exec $CMD webserver &
...
# By default we use SequentialExecutor
  ...
  sed -i "s#sql_alchemy_conn = mysql+mysqldb://root:root@mysql/airflow#sql_alchemy_conn = sqlite:////usr/local/airflow/airflow.db#" "$AIRFLOW_HOME"/airflow.cfg
  echo "Initialize database..."
  $CMD initdb
  exec $CMD webserver
fi
